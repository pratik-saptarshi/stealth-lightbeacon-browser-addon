...
var domainSchema = external_exports.enum([
  "seo",
  "performance",
  "accessibility",
  "aeo",
...
var issueCountBySeveritySchema = external_exports.object({
  critical: external_exports.number().nonnegative(),
  high: external_exports.number().nonnegative(),
  medium: external_exports.number().nonnegative(),
  low: external_exports.number().nonnegative()
});
var issueCountByDomainSchema = external_exports.object({
  seo: external_exports.number().nonnegative(),
  performance: external_exports.number().nonnegative(),
  accessibility: external_exports.number().nonnegative(),
  aeo: external_exports.number().nonnegative(),
  ux: external_exports.number().nonnegative(),
  drupal: external_exports.number().nonnegative(),
  geo: external_exports.number().nonnegative(),
  "security-headers": external_exports.number().nonnegative(),
  "WCAG2.1AA": external_exports.number().nonnegative(),
  "WCAG2.2AA": external_exports.number().nonnegative()
}).catchall(external_exports.number().nonnegative());
var issueSchema = external_exports.object({
  ...
  source: external_exports.enum(["dom-only", "backend"])
});
var engineRecommendationSchema = external_exports.object({
  engine: external_exports.enum(["http", "fast-obscura", "stealth-playwright", "mcp"]),
  reason: external_exports.string().min(1),
  confidence: external_exports.number().min(0).max(1)
});
var scanRequestSchema = external_exports.object({
  requestId: external_exports.string().min(1),
  url: external_exports.string().url(),
  tabId: external_exports.number().int().nonnegative().optional(),
  engine: external_exports.enum(["dom-lite", "crawl-lite"]),
  crawlDepth: external_exports.number().int().nonnegative().max(4).optional(),
  crawlMaxUrls: external_exports.number().int().nonnegative().max(500).optional(),
  ruleCategories: external_exports.array(domainSchema).min(1).optional(),
  backend: external_exports.object({
    enabled: external_exports.boolean().optional(),
    mode: external_exports.enum(["http", "stdin"]).optional(),
    engine: external_exports.enum(["http", "fast-obscura", "stealth-playwright", "mcp"]).optional(),
    endpoint: external_exports.string().url().optional(),
    allowedHosts: external_exports.array(external_exports.string().min(1)).optional(),
    requestSigningSecret: external_exports.string().min(1).max(256).optional(),
...
var scanSnapshotSchema = external_exports.object({
  id: external_exports.string().min(1),
...
var crawlNodeSchema = external_exports.object({
  url: external_exports.string().url(),
  depth: external_exports.number().int().nonnegative(),
  status: external_exports.enum(["queued", "running", "done", "error"]),
  errorType: external_exports.enum(["cors", "timeout", "blocked", "non_html", "other"]).optional(),
  discoveredFrom: external_exports.string().url().optional(),
  finalUrl: external_exports.string().url().optional(),
  statusCode: external_exports.number().nonnegative().optional(),
  note: external_exports.string().optional()
});
var scanResultSchema = external_exports.object({
  requestId: external_exports.string().min(1),
  snapshot: scanSnapshotSchema,
  crawlNodes: external_exports.array(crawlNodeSchema).optional(),
  recommendation: engineRecommendationSchema.optional()
});
var diffResultSchema = external_exports.object({
  newIssues: external_exports.array(issueSchema),
  resolvedIssues: external_exports.array(issueSchema),
  regressions: external_exports.array(issueSchema),
  improvements: external_exports.array(issueSchema)
});
...
var backendRulesetCategorySchema = external_exports.array(
...
var addonRulesetSchema = external_exports.object({
